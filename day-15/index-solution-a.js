...
const puzzle = (text, lineY) => {
	...
	data.forEach((entry)=>{
		...
		const straightDownAtLine = `${sensorSplits[0]},${lineY}`;
		if (insideZoneOfInhibition(
			entry.sensor,
			entry.beacon,
			straightDownAtLine
		)) {
			const distFromCenter = Math.abs(lineY - sensorSplits[1]);
	...
	const reduction = reduceArrays(linePairs);
	let count = 0;
	reduction.forEach((array)=>{
...
console.log(me.unitTestResults(
	'Sample input',
	sampleAnswer,
	puzzle(sampleFileText, sampleY)
));
console.log("real input:", puzzle(fileText, realY));
