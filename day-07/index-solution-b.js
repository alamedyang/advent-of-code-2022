...
const puzzle = (inputText) => {
	...
	const needSize = 30000000;
	const spaceTaken = getFileSizeSumRecursive(fs["/"]);
	const currentFree = totalSize - spaceTaken;
	const needFree = needSize - currentFree;

	const sortedSizes = Object.values(sizesMap)
		.sort((a,b) => a-b );
	const filteredSizes = sortedSizes.filter((item) => item >= needFree);
