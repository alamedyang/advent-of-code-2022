		...
		if (!neighborCoords.length) {
			...
			for (let i = 0; i < loopyDir.length; i++) {
				...
				if (
					...
					// coordsMap[coord].tryNext = (tryNext + i + 1) % 4;
					// oops... thought each elf had a different cycle
					break;
				}
			}
		}
...
const drawMap = (coordsMap) => {
	const fused = me.drawUnanchoredCoords(Object.keys(coordsMap))
		.split('\n')
		.reverse()
		.join('\n');
	// console.log(fused);
