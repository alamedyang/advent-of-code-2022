...
const pieceOffset = 2;
const targetRockCount = 1000000000000;
...
const fingerprints = {};
const fingerprintState = (world, windIndex, rockIndex, value) => {
	const label = `w${windIndex}:r${rockIndex}:${world.join()}`;
	const exists = fingerprints[label] !== undefined;
	if (exists) {
...
const puzzle = (text) => {
	...
	let windIndex = 0;
	let scrolled = 0;
	for (let rockIndex = 0; rockIndex < targetRockCount; rockIndex++) {
		const piece = pieces[rockIndex % pieces.length];
		...
		const printExists = fingerprintState(
			world,
			windIndex % winds.length,
			rockIndex % pieces.length,
			value
		);
		if (printExists) {
			const scrollDiff = scrolled - printExists.scrolled;
			const rockDiff = rockIndex - printExists.rockIndex;
			const loopCount = Math.floor((targetRockCount - rockIndex) / rockDiff);
			rockIndex += loopCount * rockDiff;
			scrolled += loopCount * scrollDiff;
		}
	}
